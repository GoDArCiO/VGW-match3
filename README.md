...
`.github/workflows/ci.yml` is a **GameCI** (`game-ci/unity-test-runner`) workflow that runs the EditMode
suite on a Linux container, the cloud equivalent of the local `tools\unity.ps1 test` gate. It is set to
**manual trigger** (`workflow_dispatch`) on purpose: running Unity in CI needs a Unity license stored as a
repo secret, and I didn't want to attach a personal Unity license to a showcase project like this. The
suite is verified **locally** instead, and everything is green:

```text
tools\unity.ps1 compile   ->  COMPILE OK
tools\unity.ps1 test      ->  TESTS PASS  50/50 passed, 0 skipped
```

The workflow is included to show the CI wiring; adding the `UNITY_LICENSE` / `UNITY_EMAIL` /
`UNITY_PASSWORD` repo secrets is all it takes to run it green from the Actions tab.
